...
def train_lstm_graph(stock_name, lstm_graph, config=DEFAULT_CONFIG):
    ...
    with tf.compat.v1.Session(graph=lstm_graph) as sess:
        merged_summary = tf.compat.v1.summary.merge_all()
        writer = tf.compat.v1.summary.FileWriter('_logs/' + graph_name, sess.graph)
            ...
            if epoch_step % 10 == 0:
                ...
                if epoch_step % 50 == 0:
                    print("Predictions:", [(
                        ...
                        list(map(lambda x: round(x, 4), stock_dataset.test_y[-j]))
                    ) for j in range(5)])
